...
_config_ctx: contextvars.ContextVar[dict[str, Any] | None] = contextvars.ContextVar(
    "luna_workstation_config", default=None
)
...
def reset_context_config(token: contextvars.Token[dict[str, Any] | None]) -> None:
    """Reset context-bound config using a previously returned token."""
    _config_ctx.reset(token)


@contextmanager
def config_context(config: dict[str, Any]) -> Iterator[None]:
