...
from luna_workstation.reporting import ReportGenerator
from luna_workstation.observability import configure_opentelemetry
from luna_workstation.graph.journal_mixin import JournalPersistenceMixin
...
class ResearchAgentsGraph(GraphRunContextMixin, JournalPersistenceMixin):
    ...
    def propagate(
        ...
        *,
        ...
        run_orchestrator = getattr(self, "run_orchestrator", None)
        if run_orchestrator is None:
            run_orchestrator = ResearchRunOrchestrator()
            self.run_orchestrator = run_orchestrator
        return run_orchestrator.propagate(
            self,
            company_name,
            trade_date,
    ...
    async def apropagate(
        ...
        *,
        ...
        if run_orchestrator is None:
            run_orchestrator = ResearchRunOrchestrator()
            self.run_orchestrator = run_orchestrator
        return await run_orchestrator.apropagate(
            self,
            company_name,
            trade_date,
            node_callback=node_callback,
    ...
    def _run_graph(
        ...
        *,
        ...
        if run_orchestrator is None:
            run_orchestrator = ResearchRunOrchestrator()
            self.run_orchestrator = run_orchestrator
        return run_orchestrator.run_graph(
            self,
            company_name,
            trade_date,
            node_callback=node_callback,
