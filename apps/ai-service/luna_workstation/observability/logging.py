...
_OBSERVABILITY_CONTEXT: contextvars.ContextVar[dict[str, Any]] = contextvars.ContextVar(
    "luna_workstation_observability_context",
    default={},
)

_OBS_RUN_EVENT_PERSIST: contextvars.ContextVar[dict[str, Any] | None] = (
    contextvars.ContextVar(
        "luna_workstation_obs_run_event_persist",
        default=None,
    )
)
...
def configure_plain_observability_logging(level: int | None = None) -> None:
    """Route ``luna_workstation.*`` warnings to stderr in ``--plain`` mode.
    ...
    for h in log.handlers:
        if getattr(h, "_luna_workstation_plain_stderr", False):
            h.setLevel(level)
            return
    ...
    handler._luna_workstation_plain_stderr = True  # type: ignore[attr-defined]
    handler.addFilter(SecretRedactionFilter())
    log.addHandler(handler)


def install_secret_redaction_filter() -> None:
    """Install a global redaction filter on root and luna_workstation loggers."""
    ...
    for logger_name in ("", "luna_workstation"):
