...
Here is the current conversation history: {guard_untrusted_context("risk_history", history)} Here are the last arguments from the conservative analyst: {guard_untrusted_context("last_conservative_argument", current_conservative_response)} Here are the last arguments from the neutral analyst: {guard_untrusted_context("last_neutral_argument", current_neutral_response)}. If there are no responses from the other viewpoints yet, present your own argument based on the available data.
