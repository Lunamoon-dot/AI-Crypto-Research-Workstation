...
class TestDeferredReflection:
    ...
    def test_reflect_on_final_decision_returns_llm_output(self):
        mock_llm = MagicMock()
        # Make with_structured_output raise so the Reflector falls back to free-text
        ...
        # Free-text fallback wraps the LLM output in structured headers
        assert "Directionally correct. Thesis confirmed." in result
        assert "**Directional**:" in result
        mock_llm.invoke.assert_called()

    def test_reflect_on_final_decision_includes_returns_in_prompt(self):
        """Return figures are present in the prompt sent to the LLM."""
        mock_llm = MagicMock()
        mock_llm.with_structured_output.side_effect = NotImplementedError
        ...
        # The prompt is now a single string, passed as first positional arg
        prompt = mock_llm.invoke.call_args[0][0]
        assert "-8.0%" in prompt
        assert "-5.0%" in prompt
        assert "Exit position immediately." in prompt
    ...
    def test_fetch_returns_valid_ticker(self):
        ...
        mock_graph.config = {"asset_class": "stock"}
