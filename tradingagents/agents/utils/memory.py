...
class TradingMemoryLog:
    ...
    def store_decision(
        ...
        *market_type* is ``"spot"`` or ``"swap"`` — stored in the entry
        body so downstream reflection can differentiate thesis quality
        (spot) from execution quality (futures).
        """
        ...
        meta_line = f"MARKET_TYPE: {market_type}" if market_type else ""
        body = final_trade_decision
        if meta_line:
            body = f"{meta_line}\n{body}"
    ...
    def _parse_entry(self, raw: str) -> Optional[dict]:
        ...
        entry = {
            ...
            "market_type": "",  # extracted from decision body below
        }
        body = "\n".join(lines[1:]).strip()
        decision_match = self._DECISION_RE.search(body)
        reflection_match = self._REFLECTION_RE.search(body)
        decision_text = decision_match.group(1).strip() if decision_match else ""
        # Extract MARKET_TYPE metadata if present
        if decision_text.startswith("MARKET_TYPE:"):
            first_line = decision_text.split("\n")[0]
            entry["market_type"] = first_line.replace("MARKET_TYPE:", "").strip()
            decision_text = decision_text[len(first_line):].strip()
