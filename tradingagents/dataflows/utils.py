...
# like BTC/USDT:USDT). Slash and colon are replaced with underscore
...
def safe_ticker_component(value: str, *, max_len: int = 32) -> str:
    ...
    Returns ``value`` with ``/`` replaced by ``_`` when it matches the
    allowed pattern; raises ``ValueError`` otherwise.
    """
    if not isinstance(value, str) or not value:
        raise ValueError(f"ticker must be a non-empty string, got {value!r}")
    # Block path traversal: ".." combined with "/" escapes directories
    if ".." in value:
        raise ValueError(f"ticker cannot contain '..': {value!r}")
    # Replace separators early so crypto pairs like BTC/USDT:USDT pass.
