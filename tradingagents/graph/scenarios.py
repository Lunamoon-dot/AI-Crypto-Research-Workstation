...
def build_scenarios_for_thesis(
    ...
    *,
    ...
    signals = signals or []
    scenarios = [
        _directional_confirmation(thesis, debate=debate, signals=signals),
        _invalidation_scenario(thesis, debate=debate, signals=signals),
        _neutral_wait_scenario(thesis, signals=signals),
    ]
    if _has_material_conflict(thesis, debate):
        scenarios.append(_contradiction_scenario(thesis, debate=debate))
    ...
    *,
    ...
    return Scenario(
        ...
        expected_market_behavior=(
            "The thesis quality deteriorates; preserve optionality and avoid forcing the original view."
        ),
        probability_band=(
        ...
        risk_map=_dedupe((thesis.contradictions or []) + thesis.risk_notes + contradicting)[:5],
        suggested_user_action="stand aside or reassess",
    )


def _neutral_wait_scenario(thesis: TradeThesis, *, signals: list[Signal]) -> Scenario:
    ...
    return Scenario(
        ...
        condition=condition,
        expected_market_behavior=(
            "Range or chop remains the base case; waiting for cleaner evidence is preferable."
        ),
        ...
        risk_map=_dedupe(thesis.risk_notes + ["Chop can create false confirmation signals."])[:5],
...
def _contradiction_scenario(
    ...
    *,
    ...
    return Scenario(
        ...
        risk_map=_dedupe(contradictions + thesis.risk_notes)[:5],
